...
router.get('/', async (req, res) => {
  try {
    const files = await getFiles(dirs.stories);
    const stories = await Promise.all(
      files.map(async (filename) => {
        const file = await readFile(dirs.stories, filename);
...
router.post('/', async (req, res) => {
  try {
    ...
    await writeFile(dirs.stories, filename, metadata, content);
    
    // Rebuild JSON asynchronously (don't wait)
    rebuildJSON('story').catch(err => console.error('Error rebuilding JSON:', err));
...
router.delete('/:id', async (req, res) => {
  try {
    const filename = `${req.params.id}.md`;
    await deleteFile(dirs.stories, filename);
